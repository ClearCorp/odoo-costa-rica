...
result = rules.NET > categories.NET * 0.10''',
    ...
    def satisfy_condition(self,localdict):
        ...
        hr_salary_rule_obj = self.env['hr.salary.rule'] #object from hr salary rule to use in python code
        ...
        result = super(hrRulesalary, self).satisfy_condition(localdict)
        return result
...
# =============================================================================
    def compute_total_rent(self,company,inputs,employee,
        ...
        if payslip.credit_note:
            ...
            original_payslip = payslip_obj.search([('name','=',original_name),('employee_id','=', employee.id), ('date_to', '=', payslip.date_to), ('date_from','=', payslip.date_from)])[0]
        ...
        """"Get total payments"""
        previous_payments = payslip_obj.get_qty_previous_payment(employee, payslip)
        ...
        total_paid_rent = payslip_obj.get_previous_rent(employee,payslip) #Rent already paid
        total_curr_rent = (rent_empl_total - total_paid_rent) / (future_payments + actual_payment) 

        return total_curr_rent

    def python_expresion_rent(self, company, inputs, employee, categories, payslip):
        """Function that evaluated if compute rent applies to salary rule"""
        total = self.compute_total_rent(company, inputs, employee, categories, payslip)
        if total != 0.0:
            return True
        else:
            return False
